...
export default function App() {
  ...
  return (
    <div className="h-screen bg-[#000514] text-white flex flex-col select-none overflow-hidden">
      {/* Header */}
      <header className="h-12 bg-[#100C1C] border-b border-[#1A1726] flex items-center px-4 shrink-0">
        <div className="flex items-center gap-2 flex-1">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#0ECC68] to-[#087a3d] flex items-center justify-center">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><circle cx="12" cy="5" r="3"/><circle cx="8" cy="12" r="1.5"/><circle cx="16" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/></svg>
          </div>
          <span className="text-sm font-bold">Plinko</span>
        </div>
        <div className="flex items-center gap-2">
          {lastWin && (
            <div className="anim-slide h-7 px-2.5 rounded-lg flex items-center gap-1.5 text-xs font-bold tabular-nums"
              style={{ backgroundColor: `${lastWin.color}10`, border: `1px solid ${lastWin.color}20`, color: lastWin.color }}>
              <span className="opacity-50">{lastWin.mult}x</span>
              {lastWin.amount >= 0 ? '+' : ''}{lastWin.amount.toFixed(2)}
            </div>
          )}
          {records.length > 0 && (
            <div className={`hidden md:flex h-7 px-2.5 rounded-lg items-center text-xs font-bold tabular-nums border
              ${pl >= 0 ? 'bg-[#0ECC68]/5 border-[#0ECC68]/15 text-[#0ECC68]' : 'bg-[#ff003f]/5 border-[#ff003f]/15 text-[#ff003f]'}`}>
              {pl >= 0 ? '+' : ''}{pl.toFixed(2)}
            </div>
          )}
          <div className="h-7 bg-[#000514] border border-[#1A1726] rounded-lg px-3 flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-[#FBCE04] tabular-nums">{dispBal.toFixed(2)}</span>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="border-t border-[#1A1726] bg-[#100C1C] shrink-0">
            <BetControls
        ...
        {/* History sidebar */}
        <div className="hidden lg:flex w-[220px] shrink-0 border-l border-[#1A1726]">
